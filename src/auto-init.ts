...
import { gsap, ScrollTrigger, initGSAP, refreshScrollTriggers } from './utils/gsap-config';
import { getAnimatedElements, parseAttributes, isValidAnimation } from './utils/data-attributes';
import { ready, debounce, isBelowViewport, getOptimalScrollTriggerStart } from './utils/helpers';
...
function animateElements(root: Document | HTMLElement = document): void {
  ...
  elements.forEach(element => {
    ...
    const hasTriggerMode = attrs.triggerMode === 'hover' || attrs.triggerMode === 'click';

    // Skip validation if it's a hover-only or click-only element
    if (!animation && !hasHover && !hasTriggerMode) {
      return;
    }
...
function executeOnScroll(
  ...
  const optimalStart = getOptimalScrollTriggerStart(element, config.start || 'top 80%');
  ...
  if (config.option) {
    ...
    try {
      ...
      console.error(`[GSAP Dattebayo] Error executing ${animationName} with option ${config.option}:`, error);
    }
  } else {
...
function createHoverAnimation(
  ...
  const hoverAnimations: Record<string, gsap.TweenVars> = {
    ...
    elasticZoom: { scale: 1.15, duration: duration * 1.5, ease: 'elastic.out(1, 0.3)' },
 ...
 */
function getAnimationFunction(animationName: string): ((target: HTMLElement, options: any) => any) | null {
  // Animation mapping
 ...
 */
function getInitialState(animationName: string, config: DattebayoDefaults): gsap.TweenVars {
 ...
 */
function getFinalState(animationName: string, config: DattebayoDefaults): gsap.TweenVars {
